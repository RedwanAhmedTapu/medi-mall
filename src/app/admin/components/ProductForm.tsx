...
const ProductForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const variantObjects = selectedVariants.map(variantId => 
        allVariants?.find(variant => variant._id === variantId)
      ).filter((variant): variant is Variant => variant !== undefined); // Filter out undefined values
      ...
      await addProduct({
        ...
        secondaryCategory,
        tertiaryCategory,
        variants: variantObjects, // Ensure this is an array of ProductVariant objects
      }).unwrap();
