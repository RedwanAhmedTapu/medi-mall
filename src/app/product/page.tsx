...
export default function ProductsPage() {
  ...
  const filteredProducts = selectedCategory
    ? products?.filter(product => product.primaryCategoryId.name === selectedCategory)
    : products;
            ...
            <li key={category.id} className="mb-2">
              <button
                className={`text-left w-full p-2 flex items-center justify-between rounded ${
                  selectedCategory === category.id ? 'bg-blue-500 text-white' : 'bg-white text-gray-800'
                }`}
                onClick={() => handleCategorySelect(category.id)}
              >
                {category.name}
                <FaAngleRight className={`${selectedCategory === category.id ? 'text-white' : 'text-gray-500'}`} />
              </button>
            </li>
          ))}
        </ul>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {filteredProducts.map((product) => (
              <div key={product._id} className="border rounded-lg p-4">
                <img src={product.photos[0]} alt={product.name} className="w-full h-48 object-cover mb-4" />
